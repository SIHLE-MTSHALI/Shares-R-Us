...
const PortfolioView = () => {
  ...
  const fetchChartData = useCallback(async (portfolioData, range, comparison) => {
    if (!portfolioData || !portfolioData.id) return;
    try {
  ...
  useEffect(() => {
    ...
    return () => {
      WebSocketService.offPriceUpdate(handlePriceUpdate);
    };
  }, []);
  ...
  const handleAddAsset = async (e) => {
    ...
    try {
      ...
      const assetData = {
        ...
        purchase_price: parseFloat(newAsset.purchasePrice)
      };

      const updatedPortfolio = await addAssetToPortfolio(id, assetData);
      setPortfolio(updatedPortfolio);
      setNewAsset({ symbol: '', quantity: '', purchasePrice: '' });
      toast.success('Asset added successfully');
    } catch (error) {
      if (error.response && error.response.status === 422) {
        const errorMessage = error.response.data.detail || 'Invalid input. Please check your data.';
